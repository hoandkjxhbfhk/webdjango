...
from .forms import CustomChangePasswordForm, ProfileForm
...
def profiles(request):
    ...
    return render(
        request,
        "profilesindex.html",
        {"profiles": profiles, "product_list": product_list},
    )
...
def edit_profile(request, profile_slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "{} {}'s profile updated.".format(form.cleaned_data["first_name"], form.cleaned_data["last_name"]),
            )
