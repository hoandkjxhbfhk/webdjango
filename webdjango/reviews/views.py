...
from .models import Cluster, Review, Wine
...
def user_recommendation_list(request):
    ...
    wine_list = sorted(
        list(Wine.objects.filter(id__in=other_users_reviews_wine_ids)), key=lambda x: x.average_rating(), reverse=True
    )

    return render(request, "user_recommendation_list.html", {"username": request.user.username, "wine_list": wine_list})
